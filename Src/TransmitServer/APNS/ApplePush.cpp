...
bool CApplePush::init()
{
	...
	if(pAPNS.isNull())
	{
		warnf("%s, %d: Apple Push service init get config failed.", __FILE__, __LINE__);
		return false;
	}
	...
	for(std::size_t i = 0; i < pArray->size(); i++)
	{
		...
		if(proxy == "bmob")
		{
			...
			{
				warnf("%s, %d: Apple Push service init read bmob config failed.", __FILE__, __LINE__);
				m_bmob_info = NULL;
				return false;
			}
		}
...
bool CApplePush::pushBmob(const std::string& device_token, const std::string& content)
{
	...
	infof("%s, %d: Apple Push service push notification[%s] through bmob.", __FILE__, __LINE__, dsStr.c_str());
	...
	{
		warnf("%s, %d: Apple Push service bmob receive response error[%s].", __FILE__, __LINE__, e.message().c_str());
		return false;
	}
	infof("%s, %d: Apple Push service bmob receice:%s", __FILE__, __LINE__);
	return true;
}
